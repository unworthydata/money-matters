...
data <- data %>% left_join(total_prevalence, by = c("income", "year"))
...
plot4 <- data %>% 
  plot_ly(x = ~income, y = ~relative_val, color = ~cause, colors = my_palette, type = "bar", 
          split = ~cause, groupnorm = "percent", 
          animation_frame = ~year) %>%
  layout(title = "Relative Prevalence of Mental Disorders by Income Level",
